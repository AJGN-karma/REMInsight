...
export default function Dashboard() {
  ...
  useEffect(() => {
    (async () => {
      try {
        // ✅ Query all users’ predictions via collectionGroup
        const qy = query(collectionGroup(db, "predictions"), orderBy("createdAt", "desc"), limit(500));
  ...
  const { dist, timeline, confTrend } = useMemo(() => {
    ...
    docs.forEach((d) => {
      ...
      if (probs.length) confTrend.push(Math.max(...probs));
      ...
      dayMap.get(key).push(pred);
    });
    const timeline = Array.from(dayMap.entries())
      .sort(([a], [b]) => a.localeCompare(b))
      .map(([date, arr]) => ({ date, avg: arr.reduce((x, y) => x + y, 0) / arr.length }));
          ...
          <>
            <div style={{ display: "grid", gap: 12, gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", marginBottom: 12 }}>
              ...
              <KPI
                ...
                value={confTrend.length ? (confTrend.reduce((a, b) => a + b, 0) / confTrend.length * 100).toFixed(1) + "%" : "—"}
                color="#10b981"
              />
            </div>
            ...
            <div style={card}>
              ...
              <div style={{ fontSize: 12, color: "#64748b", marginTop: 6 }}>
                Note: shows predictions saved to Firestore (via “Save to Patient History”).
              </div>
            </div>
          </>
        )}
